...
exports.hashname = function(key, args)
{
  ...
  var self = thnode.hashname(key, function(to, msg){
    ...
    {
      self.io.sockets.sockets[to.id].emit('packet', {
  ...
  self.http = function(http, io)
  {
    ...
    io.on('connection', function(socket){
      socket.on('packet', function(packet) {
        ...
        self.receive((new Buffer(packet.data, 'base64')).toString('binary'), {
          type:'bridge',
          id:socket.id
        });
      });
  ...
  function msgs4(msg, rinfo){
    self.receive(msg.toString('binary'), {
      type:'ipv4',
      ip:rinfo.address,
      port:rinfo.port
    });
  ...
  self.online = function(callback)
  {
    ...
    if(!self.seeded){
      self.addSeeds(path.join(__dirname,'seeds.json'));
    }
        ...
        function interfaces()
        {
          ...
          for (var dev in ifaces) {
            ifaces[dev].forEach(function(details){
              ...
              if(details.family == 'IPv4' && !details.internal && thnode.isLocalIP(address4.address)){
                address4.address = details.address;
              }
              if(details.family == 'IPv6' && !details.internal && thnode.isLocalIP(address6.address)){
                address6.address = details.address;
              }
            });
          }
          ...
          if(self.io && !self.paths.http){
            self.pathSet({
              type:'http',
              http:'http://'+address4.address+':'+address4.port
            });
          }
...
// use node streams
thnode.channelWraps.stream = function(chan)
{
  var duplex = chan.duplex = new require('stream').Duplex();
  ...
  function doChunk(){
    ...
    if(duplex.bufBody.length === 0 && Object.keys(duplex.bufJS).length === 0){
      return;
    }
    var bodyout;
    ...
    if(duplex.bufBody.length > 0)
    {
      ...
      if(duplex.bufBody.length < len){
        len = duplex.bufBody.length;
      }
      bodyout = duplex.bufBody.slice(0, len);
      duplex.bufBody = duplex.bufBody.slice(len);
    }
    // send it!
    sendStream(self, stream, {
      js:jsout,
      body:bodyout,
      done:function(){
  ...
  duplex.end = function(){
    ...
    if(stream.errMsg){
      duplex.bufJS.err = stream.errMsg;
    }
    doChunk();
  }
