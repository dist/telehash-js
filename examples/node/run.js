...
var vlog = fs.createWriteStream(path.join(process.cwd(),argv.v), {
  flags:'a'
});
vlog.write('starting with '+JSON.stringify(argv)+'\n');

telehash.debug(function(){
  var args = arguments;
  args = Object.keys(arguments).map(function(k){
    return args[k];
  });
...
if(fs.existsSync(idfile))
{
  ...
  telehash.genkey(function(err, key){
    if(err){
...
function messageInit(err, arg, chan, cb)
{
  ...
  chan.callback = function(err, arg, chan, cbMessage){
    if(arg && arg.js.message){
      log('m['+chan.nick+']:',arg.js.message);
    }
    if(err){
...
function groupInit(arg, chan)
{
  ...
  chan.callback = function(err, arg, chan, cbMessage){
    if(arg && arg.js.message){
      log('g['+chan.group.id+'/'+chan.nick+']: '+arg.js.message);
    }
    if(err)
    {
...
cmds.help = cmds['?'] = function(arg){
  ...
  log('"sjoin group hashname"','join a group that exists via that hashname');
  log('"sgw|gwho group"','see who\'s in the group');
